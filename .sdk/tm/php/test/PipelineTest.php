...
// DymoApiIntroduction SDK pipeline test
//
...
{
    private static function utility(): DymoApiIntroductionUtility
    {
        return new DymoApiIntroductionUtility();
    }
    ...
    private static function ctx(array $over = []): DymoApiIntroductionContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DymoApiIntroductionContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DymoApiIntroductionOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DymoApiIntroductionError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DymoApiIntroductionOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DymoApiIntroductionMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DymoApiIntroductionOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DymoApiIntroductionMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DymoApiIntroductionOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DymoApiIntroductionMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DymoApiIntroductionSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DymoApiIntroductionMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DymoApiIntroductionResponse([]),
            'result' => new DymoApiIntroductionResult([]),
        ]);
        [, $err] = DymoApiIntroductionMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DymoApiIntroductionSpec([]),
            'response' => null,
            'result' => new DymoApiIntroductionResult([]),
        ]);
        [, $err] = DymoApiIntroductionMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DymoApiIntroductionSpec(['step' => 's']),
            'response' => new DymoApiIntroductionResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DymoApiIntroductionResult([]),
        ]);
        [, $err] = DymoApiIntroductionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DymoApiIntroductionSpec(['step' => 's']),
            'response' => new DymoApiIntroductionResponse(self::resp(200, ['v' => 1])),
            'result' => new DymoApiIntroductionResult([]),
        ]);
        [, $err] = DymoApiIntroductionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DymoApiIntroductionSpec(['step' => 's']),
            'response' => new DymoApiIntroductionResponse(self::resp(200, ['v' => 2])),
            'result' => new DymoApiIntroductionResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DymoApiIntroductionMakeResponse::call($ctx);
    ...
    {
        $preset = new DymoApiIntroductionResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DymoApiIntroductionSpec([]),
            'response' => new DymoApiIntroductionResponse([]),
            'result' => new DymoApiIntroductionResult([]),
        ...
        [$got, $err] = DymoApiIntroductionMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DymoApiIntroductionResult([])]);
        [, $err] = DymoApiIntroductionMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DymoApiIntroductionSpec([]), 'result' => null]);
        [, $err] = DymoApiIntroductionMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DymoApiIntroductionSpec(['step' => 's']),
            'result' => new DymoApiIntroductionResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DymoApiIntroductionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DymoApiIntroductionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DymoApiIntroductionSpec(['step' => 's']),
            'result' => new DymoApiIntroductionResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DymoApiIntroductionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DymoApiIntroductionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DymoApiIntroductionResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DymoApiIntroductionSpec([]),
            'result' => new DymoApiIntroductionResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DymoApiIntroductionMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DymoApiIntroductionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DymoApiIntroductionError('boom', 'boom');
        $utility->fetcher = function (DymoApiIntroductionContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DymoApiIntroductionSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DymoApiIntroductionMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DymoApiIntroductionContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DymoApiIntroductionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DymoApiIntroductionResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        DymoApiIntroductionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DymoApiIntroductionContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DymoApiIntroductionSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DymoApiIntroductionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DymoApiIntroductionResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DymoApiIntroductionSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DymoApiIntroductionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DymoApiIntroductionMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DymoApiIntroductionMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DymoApiIntroductionResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DymoApiIntroductionDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DymoApiIntroductionResult(['ok' => false])]);
        $this->expectException(DymoApiIntroductionError::class);
        DymoApiIntroductionDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DymoApiIntroductionResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DymoApiIntroductionMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DymoApiIntroductionMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DymoApiIntroductionBaseFeature();
        $b = new DymoApiIntroductionBaseFeature();
        DymoApiIntroductionFeatureAdd::call($ctx, $a);
        DymoApiIntroductionFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DymoApiIntroductionBaseFeature
    {
        $f = new DymoApiIntroductionBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DymoApiIntroductionFeatureAdd::call($ctx, self::named_feature('a'));
        DymoApiIntroductionFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        DymoApiIntroductionFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        DymoApiIntroductionFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        DymoApiIntroductionFeatureAdd::call($ctx, $replace);
        ...
        DymoApiIntroductionFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DymoApiIntroductionContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DymoApiIntroductionSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DymoApiIntroductionPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        DymoApiIntroductionPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DymoApiIntroductionResponse(['headers' => null]),
            'result' => new DymoApiIntroductionResult([]),
        ]);
        DymoApiIntroductionResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        DymoApiIntroductionResultBody::call($ctx);
