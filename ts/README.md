# DymoApiIntroduction TypeScript SDK



The TypeScript SDK for the DymoApiIntroduction API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Security()` — each with a small set of operations (`create`)
...
```ts
const client = new DymoApiIntroductionSDK({ apikey: '...' })
...
```ts
new DymoApiIntroductionSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `DymoApiIntroductionSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `DymoApiIntroductionSDK.test(testopts?, sdkopts?)` | `DymoApiIntroductionSDK` | Create a test-mode client. |
...
| `client` | `client(): DymoApiIntroductionSDK` | Return the parent SDK client. |
...
```
dymo-api-introduction/
├── src/
│   ├── DymoApiIntroductionSDK.ts        # Main SDK class
