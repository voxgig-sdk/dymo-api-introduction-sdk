...
//     (\`new DymoApiIntroductionSDK(...)\` / \`DymoApiIntroductionSDK.test(...)\` become
//     \`DymoApiIntroductionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'DymoApiIntroductionSDK'
...
const TEST_SEED = {"entity":{"security":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DymoApiIntroductionSDK(...)\` and any
// \`DymoApiIntroductionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DymoApiIntroductionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DymoApiIntroductionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
