...
import { DymoApiIntroductionSDK, BaseFeature, stdutil } from '../../..'
...
describe('SecurityEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    const live = 'TRUE' === process.env.DYMO_API_INTRODUCTION_TEST_LIVE
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'security.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let client = DymoApiIntroductionSDK.test(options, extra)
  ...
  const idmapEnvVal = process.env['DYMO_API_INTRODUCTION_TEST_SECURITY_ENTID']
  ...
  const live = 'TRUE' === env.DYMO_API_INTRODUCTION_TEST_LIVE
  ...
  const setup = {
    ...
    struct,
    data: entityData,
    explain: 'TRUE' === env.DYMO_API_INTRODUCTION_TEST_EXPLAIN,
    live,
