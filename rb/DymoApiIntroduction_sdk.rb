...
require_relative 'DymoApiIntroduction_types'


class DymoApiIntroductionSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DymoApiIntroductionHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DymoApiIntroductionHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DymoApiIntroductionFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DymoApiIntroductionUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DymoApiIntroductionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DymoApiIntroductionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DymoApiIntroductionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DymoApiIntroductionError.new(
        "#{op}_allow",
        "DymoApiIntroductionSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue DymoApiIntroductionError => err
    ...
    if fetched.is_a?(Hash)
      status = DymoApiIntroductionHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DymoApiIntroductionError.new(
        "graphql_error", "DymoApiIntroductionSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = DymoApiIntroductionSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
