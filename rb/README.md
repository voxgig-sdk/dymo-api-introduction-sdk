...
The Ruby SDK for the DymoApiIntroduction API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.Security(nil).create({ "name" => "Example" }, nil)
...
result, err = client.DymoApiIntroduction(nil).load(
  { "id" => "test01" }, nil
)
