...
class SecurityEntity
  ...
  def make
    opts = @_entopts.dup
    SecurityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DymoApiIntroductionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Security, Hash] the current Security data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DymoApiIntroductionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Security fields)
  def match_get
  ...
  #
  # @param reqdata [SecurityCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Security, Hash] the created Security; raises DymoApiIntroductionError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = DymoApiIntroductionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
