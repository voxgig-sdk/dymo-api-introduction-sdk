...
module DymoApiIntroductionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DymoApiIntroductionResponse.new({})
    ...
    if fetch_err
      ...
      response = DymoApiIntroductionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DymoApiIntroductionResponse.new(fetched)
    else
