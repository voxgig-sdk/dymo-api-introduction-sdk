...
local SecurityEntity = {}
SecurityEntity.__index = SecurityEntity


function SecurityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SecurityEntity)
  self._name = "security"
  self._client = client
...
function SecurityEntity:get_name()
  return self._name
end


function SecurityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SecurityEntity.new(self._client, opts)
end
...
function SecurityEntity:mark_deleted()
  self._deleted = true
end


function SecurityEntity:deleted()
  return true == self._deleted
end


function SecurityEntity:data_set(args)
  if args ~= nil then
...
function SecurityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SecurityEntity:match_set(args)
  if args ~= nil then
...
function SecurityEntity:match_get()
...
function SecurityEntity:stream(action, args, callopts)
...
function SecurityEntity:_run_op(ctx, post_done)
  local utility = self._utility
