...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("DYMOAPIINTRODUCTION_TEST_LIVE") == "TRUE" ||
		os.Getenv("DYMOAPIINTRODUCTION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("DYMOAPIINTRODUCTION_TEST_EXPLAIN"); explain != "" {
