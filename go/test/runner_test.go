...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("DYMO_API_INTRODUCTION_TEST_LIVE") == "TRUE" ||
		os.Getenv("DYMO_API_INTRODUCTION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("DYMO_API_INTRODUCTION_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.DymoApiIntroductionSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.DymoApiIntroductionSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.DymoApiIntroductionError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.DymoApiIntroductionSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.DymoApiIntroductionError{Msg: msg, Code: code}
}
