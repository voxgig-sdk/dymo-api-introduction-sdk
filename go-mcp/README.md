...
[MCP](https://modelcontextprotocol.io) server exposing the DymoApiIntroduction SDK as
two agent tools — `dymo-api-introduction_list` and `dymo-api-introduction_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dymo-api-introduction-mcp)
make build

# 2. Provide credentials via the environment
export DYMO_API_INTRODUCTION_APIKEY=sk_live_xxx
...
```jsonc
// dymo-api-introduction_list: first page of records
{ "entity": "security" }
{ "entity": "security", "query": { } }

// dymo-api-introduction_load: one record by id
{ "entity": "security", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dymo-api-introduction-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dymo-api-introduction \
     -- "$PWD"/dist/*/dymo-api-introduction-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dymo-api-introduction_list` and `dymo-api-introduction_load` tools now appear
   in new sessions. Ask the agent to *"list security using dymo-api-introduction"*
   and it calls `dymo-api-introduction_list` with `{"entity":"security"}`.
...
```sh
export DYMO_API_INTRODUCTION_APIKEY=sk_live_xxx            # API key
export DYMO_API_INTRODUCTION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `dymo-api-introduction_list` tool
...
### Call the `dymo-api-introduction_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `dymo-api-introduction_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dymo-api-introduction_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DYMO_API_INTRODUCTION_APIKEY` | API key sent with every request. |
| `DYMO_API_INTRODUCTION_BASE` | Optional override of the API base URL. |
