...
// DymoApiIntroduction SDK retry feature
...
class DymoApiIntroductionRetryFeature extends DymoApiIntroductionBaseFeature
{
    ...
    public function init(DymoApiIntroductionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DymoApiIntroductionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DymoApiIntroductionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DymoApiIntroductionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
