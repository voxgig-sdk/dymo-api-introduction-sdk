...
// DymoApiIntroduction SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class DymoApiIntroductionTestFeature extends DymoApiIntroductionBaseFeature
{
    ...
    public function init(DymoApiIntroductionContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (DymoApiIntroductionContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = DymoApiIntroductionParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (DymoApiIntroductionContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(DymoApiIntroductionContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = DymoApiIntroductionParam::call($ctx, $k);
