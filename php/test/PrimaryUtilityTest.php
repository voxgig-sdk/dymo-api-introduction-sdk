...
// DymoApiIntroduction SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        DymoApiIntroductionSDK $client,
        DymoApiIntroductionUtility $utility
    ): DymoApiIntroductionContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new DymoApiIntroductionContext($ctxmap, null);
    ...
    private static function fixctx(DymoApiIntroductionContext $ctx, DymoApiIntroductionSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?DymoApiIntroductionError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DymoApiIntroductionError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new DymoApiIntroductionResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new DymoApiIntroductionBaseFeature();
    ...
    {
        $hook_client = DymoApiIntroductionSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends DymoApiIntroductionBaseFeature {
            public $hook_fn = null;
            public function TestHook(DymoApiIntroductionContext $ctx): void
            {
    ...
    {
        $init_client = DymoApiIntroductionSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends DymoApiIntroductionBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new DymoApiIntroductionSDK([
            'system' => [
    ...
    {
        $blocked_client = new DymoApiIntroductionSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new DymoApiIntroductionSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new DymoApiIntroductionSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = DymoApiIntroductionSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new DymoApiIntroductionOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = DymoApiIntroductionSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof DymoApiIntroductionError) ? $result->err->msg : (string)$result->err;
