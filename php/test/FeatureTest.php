...
// DymoApiIntroduction SDK feature test
//
...
class FtCtrl extends DymoApiIntroductionControl
{
...
class FtClient
{
    ...
    private DymoApiIntroductionUtility $utility;

    public function __construct(DymoApiIntroductionUtility $utility, array $options)
    {
    ...
    public function get_utility(): DymoApiIntroductionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DymoApiIntroductionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DymoApiIntroductionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DymoApiIntroductionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DymoApiIntroductionConfig::make_feature($name);
    ...
    private static function buildUrl(DymoApiIntroductionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DymoApiIntroductionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DymoApiIntroductionResponse($fetched);
        }
    ...
    private function fail(DymoApiIntroductionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DymoApiIntroductionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
