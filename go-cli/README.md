# dymo-api-introduction-cli

boru-driven command-line client **and** interactive REPL for the DymoApiIntroduction
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dymo-api-introduction-cli)
make build

# 2. See usage (words, entities, env vars)
./dymo-api-introduction-cli --help
...
DYMO_API_INTRODUCTION_BASE=https://api.example.com ./dymo-api-introduction-cli --help

# 6. No arguments -> interactive REPL
./dymo-api-introduction-cli
dymo-api-introduction> /help
dymo-api-introduction> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dymo-api-introduction-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dymo-api-introduction-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `dymo-api-introduction>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `security`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dymo-api-introduction-cli`. |
...
(`./dymo-api-introduction-cli <expr>`) and interactively (the REPL), and expressions compose the
