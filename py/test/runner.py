# DymoApiIntroduction SDK test runner
...
class DymoApiIntroductionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DymoApiIntroductionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DymoApiIntroductionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DymoApiIntroductionTestRunner.getenv("DYMO_API_INTRODUCTION_TEST_LIVE")
        override = DymoApiIntroductionTestRunner.getenv("DYMO_API_INTRODUCTION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DymoApiIntroductionTestRunner.getenv(key)
        ...
        explain = DymoApiIntroductionTestRunner.getenv("DYMO_API_INTRODUCTION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DYMO_API_INTRODUCTION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DymoApiIntroductionTestRunner._test_control is not None:
            return DymoApiIntroductionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DymoApiIntroductionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DymoApiIntroductionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DymoApiIntroductionTestRunner.load_test_control()
...
def load_env_local():
    DymoApiIntroductionTestRunner.load_env_local()


def env_override(m):
    return DymoApiIntroductionTestRunner.env_override(m)


def entity_data(v):
    return DymoApiIntroductionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DymoApiIntroductionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DymoApiIntroductionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DymoApiIntroductionTestRunner.load_test_control()


def live_delay_ms():
    return DymoApiIntroductionTestRunner.live_delay_ms()
