...
from dymoapiintroduction_sdk.core.helpers import get_ctx_prop, to_map
...
class DymoApiIntroductionContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = DymoApiIntroductionControl()
        ...
        if isinstance(sp, DymoApiIntroductionSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, DymoApiIntroductionResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, DymoApiIntroductionResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return DymoApiIntroductionOperation({})
    ...
    def make_error(self, code, msg):
        return DymoApiIntroductionError(code, msg, self)
